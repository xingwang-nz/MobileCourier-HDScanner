...
public class DragSortListView extends ListView {
	...
	final int defaultDuration = 150;
	int removeAnimDuration = defaultDuration; // ms
	int dropAnimDuration = defaultDuration; // ms

	if (attrs != null) {
	    final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.DragSortListView, 0, 0);
	    ...
	    final float frac = a.getFloat(R.styleable.DragSortListView_drag_scroll_start, mDragUpScrollStartFrac);
	    ...
	    final boolean useDefault = a.getBoolean(R.styleable.DragSortListView_use_default_controller, true);

	    if (useDefault) {
		final boolean removeEnabled = a.getBoolean(R.styleable.DragSortListView_remove_enabled, false);
		final int removeMode = a.getInt(R.styleable.DragSortListView_remove_mode, DragSortController.FLING_REMOVE);
		final boolean sortEnabled = a.getBoolean(R.styleable.DragSortListView_sort_enabled, true);
		final int dragInitMode = a.getInt(R.styleable.DragSortListView_drag_start_mode, DragSortController.ON_DOWN);
		final int dragHandleId = a.getResourceId(R.styleable.DragSortListView_drag_handle_id, 0);
		final int flingHandleId = a.getResourceId(R.styleable.DragSortListView_fling_handle_id, 0);
		final int clickRemoveId = a.getResourceId(R.styleable.DragSortListView_click_remove_id, 0);
		final int bgColor = a.getColor(R.styleable.DragSortListView_float_background_color, Color.BLACK);

		final DragSortController controller = new DragSortController(this, dragHandleId, dragInitMode, removeMode, clickRemoveId, flingHandleId);
    ...
    private int getItemHeight(final int position) {
	final View v = getChildAt(position - getFirstVisiblePosition());
	...
	public HeightCache(final int size) {
	 ...
	 */
	public void add(final int position, final int height) {
	    final int currHeight = mMap.get(position, -1);
	...
	final int childHeight = getChildHeight(position);
	final int itemHeight = getItemHeight(position);
	...
	if (mSecondExpPos <= mSrcPos) {
		...
		if (position == mSrcPos) {
		    ...
		    final int blankHeight = itemHeight - childHeight;
	...
	final int startTop = startView.getTop();
	...
	public SmoothAnimator(final float smoothness, final int duration) {
	    mAlpha = smoothness;
	    mDurationF = duration;
	...
	public float transform(final float frac) {
	    if (frac < mAlpha) {
		return mA * frac * frac;
	...
	public void onUpdate(final float frac, final float smoothFrac) {
	    // stub
	}
	...
	public void run() {
	    ...
	    final float fraction = (SystemClock.uptimeMillis() - mStartTime) / mDurationF;
	...
	public LiftAnimator(final float smoothness, final int duration) {
	...
	public DropAnimator(final float smoothness, final int duration) {
	    super(smoothness, duration);
	}
	...
	private int getTargetY() {
	    ...
	    final View v = getChildAt(mDropPos - first);
	    int targetY = -1;
	    if (v != null) {
		if (mDropPos == srcPos) {
		    targetY = v.getTop();
	...
	public void onUpdate(final float frac, final float smoothFrac) {
	    ...
	    if (mUseRemoveVelocity) {
		final float dt = (float) (SystemClock.uptimeMillis() - mStartTime) / 1000;
		if (dt == 0) {
		    return;
		}
		final float dx = mRemoveVelocityX * dt;
     ...
     * @param velocityX
     */
    public void removeItem(final int which, final float velocityX) {
	if (mDragState == IDLE || mDragState == DRAGGING) {
		...
		mFloatPos = mSrcPos;
		final View v = getChildAt(mSrcPos - getFirstVisiblePosition());
		if (v != null) {
     ...
     */
    private void doRemoveItem(final int which) {
	// must set to avoid cancelDrag being called from the
	// DataSetObserver
	mDragState = REMOVING;
    ...
    public boolean stopDragWithVelocity(final boolean remove, final float velocityX) {

	mUseRemoveVelocity = true;
	return stopDrag(remove, velocityX);
    }

    public boolean stopDrag(final boolean remove, final float velocityX) {
	if (mFloatView != null) {
	    mDragScroller.stopScrolling(true);
    ...
    private void adjustItem(final int position) {
	final View v = getChildAt(position - getFirstVisiblePosition());
     ...
     */
    private void adjustItem(final int position, final View v, final boolean invalidChildHeight) {
    ...
    private int getChildHeight(final int position) {
	if (position == mSrcPos) {
	    return 0;
	}
    ...
    private int getChildHeight(final int position, final View item, final boolean invalidChildHeight) {
	if (position == mSrcPos) {
	    return 0;
	}
    ...
    private int calcItemHeight(final int position, final View item, final boolean invalidChildHeight) {
    ...
    private int adjustScroll(final int movePos, final View moveItem, final int oldFirstExpPos, final int oldSecondExpPos) {
	int adjust = 0;

	final int childHeight = getChildHeight(movePos);

	final int moveHeightBefore = moveItem.getHeight();
	final int moveHeightAfter = calcItemHeight(movePos, childHeight);
    ...
    private void measureItem(final View item) {
	ViewGroup.LayoutParams lp = item.getLayoutParams();
	if (lp == null) {
	    lp = new AbsListView.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
	    item.setLayoutParams(lp);
	}
	final int wspec = ViewGroup.getChildMeasureSpec(mWidthMeasureSpec, getListPaddingLeft() + getListPaddingRight(), lp.width);
	int hspec;
	if (lp.height > 0) {
     ...
     */
    public boolean startDrag(final int position, final int dragFlags, final int deltaX, final int deltaY) {
     ...
     *         in progress.
     */
    public boolean startDrag(final int position, final View floatView, final int dragFlags, final int deltaX, final int deltaY) {
    ...
    private void doDragFloatView(final boolean forceInvalidate) {
	final int movePos = getFirstVisiblePosition() + getChildCount() / 2;
	final View moveItem = getChildAt(getChildCount() / 2);
    ...
    private void doDragFloatView(final int movePos, final View moveItem, final boolean forceInvalidate) {
	...
	final boolean updated = updatePositions();

	if (updated) {
	    adjustAllItems();
	    final int scroll = adjustScroll(movePos, moveItem, oldFirstExpPos, oldSecondExpPos);
     ...
     * @param to
     */
    public void moveCheckState(final int from, final int to) {
	...
	// X . X X X . X X
	final SparseBooleanArray cip = getCheckedItemPositions();
	int rangeStart = from;
	int rangeEnd = to;
	if (to < from) {
	    rangeStart = to;
	...
	final int[] runStart = new int[cip.size()];
	final int[] runEnd = new int[cip.size()];
	final int runCount = buildRunList(cip, rangeStart, rangeEnd, runStart, runEnd);
     ...
     * @param position
     */
    public void removeCheckState(final int position) {
	final SparseBooleanArray cip = getCheckedItemPositions();
	...
	final int[] runStart = new int[cip.size()];
	final int[] runEnd = new int[cip.size()];
	final int rangeStart = position;
	final int rangeEnd = cip.keyAt(cip.size() - 1) + 1;
	final int runCount = buildRunList(cip, rangeStart, rangeEnd, runStart, runEnd);
	for (int i = 0; i != runCount; i++) {
    ...
    private static int buildRunList(final SparseBooleanArray cip, final int rangeStart, final int rangeEnd, final int[] runStart, final int[] runEnd) {
	int runCount = 0;
    ...
    private static int rotate(int value, final int offset, final int lowerBound, final int upperBound) {
	final int windowSize = upperBound - lowerBound;
    ...
    private static int findFirstSetIndex(final SparseBooleanArray sba, final int rangeStart, final int rangeEnd) {
	final int size = sba.size();
    ...
    private static int insertionIndexForKey(final SparseBooleanArray sba, final int key) {
	int low = 0;
	int high = sba.size();
	while (high - low > 0) {
	...
	public void appendState() {
	    ...
	    mBuilder.append("    <SecondExpBlankHeight>").append(getItemHeight(mSecondExpPos) - getChildHeight(mSecondExpPos))
